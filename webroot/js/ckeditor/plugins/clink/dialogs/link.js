...
CKEDITOR.dialog.add( 'link', function( editor ) {
	var plugin = CKEDITOR.plugins.clink;
	...
	var targetChanged = function() {
			...
			switch ( value ) {
				case 'frame':
					targetName.setLabel( editor.lang.clink.targetFrameName );
					targetName.getElement().show();
					break;
				case 'popup':
					popupFeatures.show();
					targetName.setLabel( editor.lang.clink.targetPopupName );
					targetName.getElement().show();
					break;
				default:
	...
	var parseLink = function( editor, element ) {
			...
			if ( !retval.type ) {
				...
				if (element) {
					retval.clink = {};
					var clinkAttr = function( inputName, attrName ) {
							var value = element.getAttribute( attrName );
							if ( value !== null )
								retval.clink[ inputName ] = value || '';
						};
					clinkAttr( 'type', 'data-clink-type' );
					clinkAttr( 'slug', 'data-clink-slug' );
					clinkAttr( 'confirm', 'data-clink-confirm' );
				}
			}
			...
			if ( element ) {
				...
				CKEDITOR.plugins.clink.synAnchorSelector ? retval.adv.advCSSClasses = getLinkClass( element ) : advAttr( 'advCSSClasses', 'class' );
			...
			if ( CKEDITOR.plugins.clink.emptyAnchorFix ) {
			...
			if ( CKEDITOR.plugins.clink.fakeAnchor ) {
				var imgs = editor.document.getElementsByTag( 'img' );
				for ( i = 0, count = imgs.count(); i < count; i++ ) {
					if ( ( item = CKEDITOR.plugins.clink.tryRestoreFakeAnchor( editor, imgs.getItem( i ) ) ) )
	...
	return {
				...
				children: [
					...
					children: [
						...
						onKeyUp: function() {
							...
							// update clinks
							// extract type and slug from url
							var urlClinkRegex = /\/type:([^\/]*)\/slug:([^\/$]*)/;
							var urlClinkMatch = url.match(urlClinkRegex);
							var clinkType = this.getDialog().getContentElement('info','clinkType');
							var clinkSlug = this.getDialog().getContentElement('info','clinkSlug');
							if (urlClinkMatch) {
								var currentType = clinkType.getValue();
								var currentSlug = clinkSlug.getValue();
								if (currentType != urlClinkMatch[1]) {
									clinkType.setValue(urlClinkMatch[1]);
								}
								if (currentSlug != urlClinkMatch[2]) {
									clinkSlug.setValue(urlClinkMatch[2]);
								}
							} else {
								clinkType.setValue('');
								clinkSlug.setValue('');
					...
					children: [{
						...
						children: [{
							...
							onKeyUp: function() {
								...
								var clinkType = this.getValue();
								var clinkSlug = this.getDialog().getContentElement('info','clinkSlug').getValue();

								if (clinkType || clinkSlug) {
									var url = this.getDialog().getContentElement('info','url');
									var currentVal = url.getValue();
									var newVal = '/type:' + clinkType + '/slug:' + clinkSlug;
							...
							commit: function( data ) {
								if (!data.clink)
									data.clink = {};
								data.clink.type = this.getValue() || '';
							},
							setup: function( data ) {
								if ( data.clink )
									this.setValue( data.clink.type || '' );
							}
						},{
							type: 'text',
							id: 'clinkSlug',
							...
							onKeyUp: function() {
								...
								var clinkSlug = this.getValue();
								var clinkType = this.getDialog().getContentElement('info','clinkType').getValue();
							...
							commit: function( data ) {
								if (!data.clink)
									data.clink = {};
								data.clink.slug = this.getValue() || '';
							},
							setup: function( data ) {
								if ( data.clink )
									this.setValue( data.clink.slug || '' );
							}
						}]
					}, {
						type: 'hbox',
						...
						children: [{
							...
							filebrowser: {
								action: 'Browse',
								target: 'info:url',
								url: Croogo.Wysiwyg.articlesPath
							},
						...
						children: [{
							...
							commit: function( data ) {
								if (!data.clink)
									data.clink = {};
								data.clink.confirm = this.getValue() || false;
							},
							setup: function( data ) {
								if (data.clink)
									this.setValue(data.clink.confirm);
							}
						}]
					}]
				}]
		...
		onOk: function() {
			...
			if (data.clink) {
				var clinkAttr = function(inputName, attrName) {
					var value = data.clink[inputName];
					if (value)
						attributes[attrName] = value;
					else
						removeAttributes.push(attrName);
			...
			if ( !this._.selectedElement ) {
				...
				if ( data.adv && data.adv.advName && CKEDITOR.plugins.clink.synAnchorSelector )
