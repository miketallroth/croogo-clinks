...
CKEDITOR.dialog.add( 'anchor', function( editor ) {
	...
	return {
		title: editor.lang.clink.anchor.title,
		minWidth: 300,
		minHeight: 60,
		onOk: function() {
		...
		onShow: function() {
			...
			if ( fullySelected ) {
				if ( CKEDITOR.plugins.clink.fakeAnchor ) {
					var realElement = CKEDITOR.plugins.clink.tryRestoreFakeAnchor( editor, fullySelected );
				...
				partialSelected = CKEDITOR.plugins.clink.getSelectedLink( editor );
				if ( partialSelected ) {
		...
		contents: [
			{
			id: 'info',
			label: editor.lang.clink.anchor.title,
			accessKey: 'I',
			elements: [
				{
				type: 'text',
				id: 'txtName',
				label: editor.lang.clink.anchor.name,
				required: true,
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.clink.anchor.errorName );
						return false;
					}
					return true;
				}
